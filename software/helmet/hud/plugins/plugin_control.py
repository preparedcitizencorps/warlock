...
class PluginControlPanel(HUDPlugin):
    ...
    def _draw_semi_transparent_background(
        self, frame: np.ndarray, panel_x: int, panel_y: int, panel_width: int, panel_height: int
    ):
        overlay = frame.copy()
        cv2.rectangle(overlay, (panel_x, panel_y), (panel_x + panel_width, panel_y + panel_height), self.bg_color, -1)
        cv2.addWeighted(overlay, self.PANEL_BACKGROUND_ALPHA, frame, 1.0 - self.PANEL_BACKGROUND_ALPHA, 0, frame)

    def _draw_panel_border(self, frame: np.ndarray, panel_x: int, panel_y: int, panel_width: int, panel_height: int):
        cv2.rectangle(
            frame,
            (panel_x, panel_y),
            (panel_x + panel_width, panel_y + panel_height),
            self.border_color,
            self.PANEL_BORDER_THICKNESS,
        )
    ...
    def _draw_title(self, frame: np.ndarray, panel_x: int, panel_y: int):
        ...
        cv2.putText(
            frame,
            title,
            (panel_x + self.PLUGIN_NAME_X_OFFSET, panel_y + self.TITLE_Y_OFFSET),
            cv2.FONT_HERSHEY_SIMPLEX,
            self.TITLE_FONT_SCALE,
            self.text_color,
            self.TITLE_THICKNESS,
        )
    ...
    def _draw_auto_reload_status(self, frame: np.ndarray, panel_x: int, panel_y: int, panel_width: int):
        status_text = self._format_auto_reload_status()
        status_color = self._get_auto_reload_status_color()
        cv2.putText(
            frame,
            status_text,
            (panel_x + panel_width - self.AUTO_RELOAD_STATUS_X_OFFSET, panel_y + self.TITLE_Y_OFFSET),
            cv2.FONT_HERSHEY_SIMPLEX,
            self.AUTO_RELOAD_FONT_SCALE,
            status_color,
            self.AUTO_RELOAD_THICKNESS,
        )
    ...
    def _draw_selection_highlight(self, frame: np.ndarray, panel_x: int, panel_width: int, y_offset: int):
        cv2.rectangle(
            frame,
            (panel_x + self.SELECTION_PADDING_X, y_offset - self.SELECTION_PADDING_Y),
            (
                panel_x + panel_width - self.SELECTION_PADDING_X,
                y_offset + self.PLUGIN_LINE_HEIGHT - self.SELECTION_PADDING_Y,
            ),
            self.selected_color,
            self.SELECTION_BORDER_THICKNESS,
        )

    def _get_plugin_name_color(self, is_selected: bool) -> tuple:
        return self.text_color if is_selected else self.disabled_color

    def _draw_plugin_name(self, frame: np.ndarray, plugin_name: str, panel_x: int, y_offset: int, name_color: tuple):
        cv2.putText(
            frame,
            plugin_name,
            (panel_x + self.PLUGIN_NAME_X_OFFSET, y_offset + self.PLUGIN_NAME_Y_OFFSET),
            cv2.FONT_HERSHEY_SIMPLEX,
            self.PLUGIN_NAME_FONT_SCALE,
            name_color,
            self.PLUGIN_NAME_THICKNESS,
        )
    ...
    def _draw_enabled_status(self, frame: np.ndarray, plugin: Dict, panel_x: int, y_offset: int):
        ...
        cv2.putText(
            frame,
            status_text,
            (panel_x + self.STATUS_ENABLED_X, y_offset + self.PLUGIN_NAME_Y_OFFSET),
            cv2.FONT_HERSHEY_SIMPLEX,
            self.STATUS_FONT_SCALE,
            status_color,
            self.STATUS_THICKNESS,
        )
    ...
    def _draw_visible_status(self, frame: np.ndarray, plugin: Dict, panel_x: int, y_offset: int):
        ...
        cv2.putText(
            frame,
            status_text,
            (panel_x + self.STATUS_ENABLED_X + self.STATUS_VISIBLE_X_OFFSET, y_offset + self.PLUGIN_NAME_Y_OFFSET),
            cv2.FONT_HERSHEY_SIMPLEX,
            self.STATUS_FONT_SCALE,
            status_color,
            self.STATUS_THICKNESS,
    ...
    def _draw_plugin_entry(
        self, frame: np.ndarray, plugin: Dict, plugin_index: int, panel_x: int, panel_width: int, y_offset: int
    ):
    ...
    def _draw_plugins_list(self, frame: np.ndarray, plugins: List[Dict], panel_x: int, panel_y: int, panel_width: int):
        if not plugins:
            cv2.putText(
                frame,
                "No plugins loaded",
                (panel_x + self.PLUGIN_NAME_X_OFFSET, panel_y + self.PLUGIN_LIST_Y_START + 10),
                cv2.FONT_HERSHEY_SIMPLEX,
                self.PLUGIN_NAME_FONT_SCALE,
                self.text_color,
                self.PLUGIN_NAME_THICKNESS,
        ...
        for plugin_index, plugin in enumerate(plugins):
            self._draw_plugin_entry(frame, plugin, plugin_index, panel_x, panel_width, y_offset)
    ...
    def _draw_help_section(self, frame: np.ndarray, panel_x: int, panel_y: int, panel_height: int):
        ...
        for text_index, text in enumerate(help_texts):
            ...
            cv2.putText(
                frame,
                text,
                (panel_x + self.PLUGIN_NAME_X_OFFSET, help_y + text_index * self.HELP_LINE_SPACING),
                cv2.FONT_HERSHEY_SIMPLEX,
                font_scale,
                self.border_color,
                thickness,
    ...
    def render(self, frame: np.ndarray) -> np.ndarray:
        ...
        panel_width, panel_height, panel_x, panel_y = self._calculate_panel_dimensions(frame_width, frame_height)

        self._draw_semi_transparent_background(frame, panel_x, panel_y, panel_width, panel_height)
