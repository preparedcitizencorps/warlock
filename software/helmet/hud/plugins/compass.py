...
class CompassPlugin(HUDPlugin):
    ...
    def _get_border_padding_data(self) -> dict:
        return self.get_data('border_padding', {
            ...
            'padding_right': 0,
            'bounds': {
                'width': self.context.frame_width,
                'x_min': 0,
                'x_max': self.context.frame_width
            }
        })
    ...
    def _draw_compass_line(self, frame: np.ndarray, bar_x_start: int,
                          bar_x_end: int, bar_y: int):
        cv2.line(frame, (bar_x_start, bar_y), (bar_x_end, bar_y),
                self.secondary_color, self.COMPASS_LINE_THICKNESS, cv2.LINE_AA)
    ...
    def _calculate_marker_x_position(self, relative_angle: float,
                                    center_x: int, pixels_per_degree: float) -> int:
    ...
    def _draw_direction_tick_mark(self, frame: np.ndarray, marker_x: int,
                                  bar_y: int, tick_height: int):
        cv2.line(frame, (marker_x, bar_y - tick_height), (marker_x, bar_y),
                self.primary_color, self.TICK_MARK_THICKNESS, cv2.LINE_AA)

    def _draw_direction_label(self, frame: np.ndarray, direction: str,
                             marker_x: int, bar_y: int, tick_height: int,
                             font_scale: float):
        text_size = cv2.getTextSize(direction, cv2.FONT_HERSHEY_SIMPLEX,
                                   font_scale, 1)[0]
        text_x = marker_x - text_size[0] // 2
        cv2.putText(frame, direction, (text_x, bar_y - tick_height - self.LABEL_Y_OFFSET),
                   cv2.FONT_HERSHEY_SIMPLEX, font_scale,
                   self.primary_color, 1, cv2.LINE_AA)

    def _draw_direction_marker(self, frame: np.ndarray, direction: str,
                              direction_angle: float, center_x: int,
                              bar_y: int, pixels_per_degree: float):
        relative_angle = self._calculate_relative_angle(direction_angle)

        if not self._is_direction_visible(relative_angle):
            return

        marker_x = self._calculate_marker_x_position(relative_angle, center_x,
                                                     pixels_per_degree)
        ...
        self._draw_direction_label(frame, direction, marker_x, bar_y,
                                  tick_height, font_scale)
    ...
    def _draw_all_direction_markers(self, frame: np.ndarray, center_x: int,
                                   bar_y: int, bar_width: int):
        pixels_per_degree = bar_width / (2 * self.VISIBLE_DEGREES_PER_SIDE)
        directions = self._get_all_compass_directions()

        for direction, angle in directions:
            self._draw_direction_marker(frame, direction, angle, center_x,
                                       bar_y, pixels_per_degree)
    ...
    def _draw_friendly_unit_marker(self, frame: np.ndarray, marker_x: int, bar_y: int):
        ...
        cv2.polylines(frame, [diamond_points], True, (255, 255, 255),
                     self.FRIENDLY_OUTLINE_THICKNESS, cv2.LINE_AA)

    def _draw_all_friendly_unit_markers(self, frame: np.ndarray, center_x: int,
                                       bar_y: int, pixels_per_degree: float):
        visible_units = self._get_visible_friendly_units()

        for unit, relative_bearing in visible_units:
            marker_x = self._calculate_marker_x_position(relative_bearing, center_x,
                                                        pixels_per_degree)
    ...
    def _calculate_heading_text_position(self, center_x: int) -> int:
        ...
        text_size = cv2.getTextSize(heading_text, cv2.FONT_HERSHEY_SIMPLEX,
                                   self.HEADING_FONT_SCALE, self.HEADING_TEXT_THICKNESS)[0]
    ...
    def _draw_heading_with_glow(self, frame: np.ndarray, center_x: int, bar_y: int):
        ...
        cv2.putText(frame, heading_text, (text_x, text_y),
                   cv2.FONT_HERSHEY_SIMPLEX, self.HEADING_FONT_SCALE,
                   self.secondary_color, self.HEADING_GLOW_THICKNESS, cv2.LINE_AA)
        cv2.putText(frame, heading_text, (text_x, text_y),
                   cv2.FONT_HERSHEY_SIMPLEX, self.HEADING_FONT_SCALE,
                   self.primary_color, self.HEADING_TEXT_THICKNESS, cv2.LINE_AA)
