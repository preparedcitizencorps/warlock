...
class BorderPaddingPlugin(HUDPlugin):
    ...
    def _draw_boundary_rectangle(self, frame: np.ndarray, x_min: int, x_max: int,
                                 y_min: int, y_max: int):
        cv2.rectangle(frame, (x_min, y_min), (x_max, y_max),
                     self.boundary_color, self.boundary_thickness, cv2.LINE_AA)

    def _draw_corner_markers(self, frame: np.ndarray, x_min: int, x_max: int,
                            y_min: int, y_max: int):
        corner_configs = [
            (x_min, y_min, [(1, 0), (0, 1)]),
            (x_max, y_min, [(-1, 0), (0, 1)]),
        ...
        for cx, cy, directions in corner_configs:
            for dx, dy in directions:
                ...
                cv2.line(frame, (cx, cy), (end_x, end_y),
                        self.boundary_color, self.boundary_thickness + 1, cv2.LINE_AA)

    def _draw_padding_measurement_text(self, frame: np.ndarray, text: str,
                                      x: int, y: int):
        font_scale = 0.4
        font_thickness = 1
        cv2.putText(frame, text, (x, y), cv2.FONT_HERSHEY_SIMPLEX,
                   font_scale, self.boundary_color, font_thickness, cv2.LINE_AA)

    def _calculate_centered_text_position(self, text: str,
                                         center_x: int, center_y: int) -> tuple:
        font_scale = 0.4
        font_thickness = 1
    ...
    def _draw_all_padding_measurements(self, frame: np.ndarray):
        measurements = [
            ...
            (f"{self.padding_bottom}px", self.context.frame_width // 2,
             self.context.frame_height - self.padding_bottom // 2),
            (f"{self.padding_left}px", self.padding_left // 2, self.context.frame_height // 2),
            (f"{self.padding_right}px", self.context.frame_width - self.padding_right // 2,
